@php
    /**
     * Gemeinsames Layout für die periodischen Berichte (Woche und Monat).
     *
     * @var \App\ValueObjects\WeeklyReportData|\App\ValueObjects\MonthlyReportData $data
     * @var string $title
     * @var string $periodLabel
     * @var string $comparisonLabel   z. B. „ggü. Vormonat"
     * @var string $trendTitle
     * @var string $legendCurrent
     * @var string $legendPrevious
     * @var array<int, string> $lineLabels
     * @var string|null $renderRoute   gesetzt = Vorschau-Toolbar anzeigen
     * @var string|null $trendSubline  optionale Zeile unter dem Verlaufs-Titel
     */
...
@if($renderRoute)
    ...
    <script>
        document.querySelector('[data-render-png]')?.addEventListener('click', async (e) => {
            ...
            try {
                const res = await fetch(@json($renderRoute), {
                    method: 'POST',
...
<div class="max-w-[1052px] mx-auto px-6 py-8">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            ...
            <p class="text-sm text-white/60">{{ $periodLabel }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-8">
        <div class="bg-white/5 rounded-xl p-6">
            ...
            <div class="mt-2 text-sm">{!! $deltaBadge($deltas['total_conversions'] ?? null) !!} <span class="text-white/40">{{ $comparisonLabel }}</span></div>
        </div>
        <div class="bg-white/5 rounded-xl p-6">
            ...
            <div class="mt-2 text-sm">{!! $deltaBadge($deltas['traffic'] ?? null) !!} <span class="text-white/40">{{ $comparisonLabel }}</span></div>
        </div>
    </div>
    ...
    <div class="bg-white/5 rounded-xl p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h2 class="text-sm font-semibold text-white/80">{{ $trendTitle }}</h2>
                @if ($trendSubline)
                    <p class="mt-1 text-xs text-white/50">{{ $trendSubline }}</p>
                @endif
            </div>
            <div class="flex items-center gap-4 text-xs">
                <span class="flex items-center gap-1.5"><span class="inline-block w-3 h-0.5 bg-primary"></span><span class="text-white/70">{{ $legendCurrent }}</span></span>
                <span class="flex items-center gap-1.5"><span class="inline-block w-3 h-0.5 bg-white/40" style="border-top:1px dashed"></span><span class="text-white/70">{{ $legendPrevious }}</span></span>
            </div>
        </div>
        @include('reports.charts._line', [
            'current' => $data->dailySeriesCurrent,
            'previous' => $data->dailySeriesPrevious,
            'labels' => $lineLabels,
        ])
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-6">
        <div class="bg-white/5 rounded-xl p-6">
            ...
            @include('reports.charts._donut', ['domains' => $data->domainsTop5])
        </div>
        <div class="bg-white/5 rounded-xl p-6">
            <h2 class="text-sm font-semibold text-white/80 mb-4">Aktivität pro Stunde</h2>
            @include('reports.charts._bar', ['hours' => $data->hourlyDistribution])
        </div>
    </div>
